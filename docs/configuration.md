...
[`docs/adr/0003-config-file-design.md`](./adr/0003-config-file-design.md) and
...
- **User-wide**: `$XDG_CONFIG_HOME/sumiq/config.yaml` if `XDG_CONFIG_HOME` is
  set, otherwise `$HOME/.config/sumiq/config.yaml`. macOS uses `~/.config` too
...
config (`--config ./sumiq.yaml` from CI). As stated in the README, this is not
a security boundary; it guards against accidents, not against the person typing
the path.
...
| `redash.api_key` / `api_key_command` | replaced **as a pair** by the strongest layer that specifies either. Both in one layer is an error |
| `query.*` | override |
...
| `redash.api_key` / `api_key_command` in a **git-tracked** file | See [`redash`](#redash) |
...
Three ways to provide it, in ascending precedence:

1. `SUMIQ_REDASH_API_KEY`
2. `redash.api_key` in `sumiq.local.yaml`
3. `redash.api_key_command` in `sumiq.local.yaml`

Precedence is just layer precedence — the environment variable sits in layer 5
and therefore wins over any file. Within a layer, `api_key` and
`api_key_command` are **mutually exclusive**; specifying both in one file is an
error rather than a silent pick. Across layers they are replaced as a pair, so
a shared `api_key_command` plus a local `api_key` leaves only the local
`api_key`.
...
- On failure, the command's stderr is included in the error message.
...
| Metacharacters | `*` (any run, including `/` and newlines), `?` (any single character) | Go `regexp` syntax |
...
**`partial` and the `keep` options.** `keep` states the condition under which
part of the value may survive; a value that doesn't meet it is hidden entirely.

- `keep: domain` keeps the part after the last `@`, **but only if it looks like
  a hostname** — two or more dot-separated labels of alphanumerics, `-`, and
  `_`. `user@example.com` → `****@example.com`, but `notanemail`,
  `@taro_handle`, `user@localhost`, and free text containing an address all
  become `****`. A pattern like `*mail*` also hits `mail_body`; keeping
  everything after `@` unconditionally would dump the body of a free-text
  column. The cost of requiring a dot is that TLD-less addresses lose their
  domain.
...
- **Several matching `partial` rules intersect**, they don't compete:
  `keep_prefix` takes the smaller value, and `keep: domain` applies only if
  every matching rule asked for it. Adding a rule can therefore never reveal
  more.
...
| **Merge** | `default_action` monotonicity, `method: none` / exempt functions / `alias_guard: off` restricted to reviewed layers, duplicate exempt names, data source name and `id` collisions, per-data-source `default_action` vs. global, rules weaker than `redact` in a `redact` scope, `api_key` + `api_key_command` in one layer |
