...
capacity の範囲内では新しい配列を確保せず元の配列に書き込むため、**呼び出し元が
append しただけで、切り詰める前の入力側の値まで書き換わる。**

```go
// 悪い: out が append されると、cap の範囲で src の続きを踏む
out := src[:n]

// 良い: 3つ目のインデックスで cap を len に固定し、配列を分離する
out := src[:n:n]
```
